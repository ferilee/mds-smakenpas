...
export function VideoManagement() {
    ...
    const handleToggleActive = async (video: Video) => {
        try {
            ...
            if (res.ok) {
                setVideos(videos.map(v => v.id === video.id ? { ...v, active: !v.active } : v));
            }
        } catch (err) {
        ...
        <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 overflow-hidden">
            <div className="flex items-center justify-between gap-2 mb-4">
                ...
                <button
                    ...
                    className={`inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-xs font-bold transition ${isAdding
            ...
            {isAdding && (
                <div className="mb-6 rounded-xl border border-brand-100 bg-brand-50/50 p-4 dark:border-brand-900/30 dark:bg-brand-900/10 animate-in slide-in-from-top-2 duration-200">
                    <form onSubmit={handleAddVideo} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {loading ? (
                            ...
                            videos.map(video => (
                                <tr key={video.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                <p className="font-semibold text-slate-900 dark:text-slate-100 truncate max-w-[200px] sm:max-w-[300px]" title={video.title}>
                                                    {video.title}
                                                </p>
                                    ...
                                    <td className="px-4 py-3">
                                        <button
                                            ...
                                            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold transition-all ${video.active
            ...
            {!loading && videos.length > 0 && (
                <div className="mt-3 flex items-center justify-between text-[10px] text-slate-500 px-1">
                    ...
                    <p className="flex items-center gap-1">
                        <Eye className="h-3 w-3" /> Klik status untuk mengaktifkan/nonaktifkan
                    </p>
                </div>
            )}
        </section>
